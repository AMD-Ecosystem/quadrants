...
class Kernel(FuncBase):
    ...
    def __init__(self, _func: Callable, autodiff_mode: AutodiffMode, _is_classkernel=False) -> None:
        ...
        # Fork-specific: per-kernel LLVM function attributes (set via @qd.kernel(fn_attrs=...)). Backend -> attr-map.
        # Mixed into the offline-cache key in `quadrants/analysis/offline_cache_util.cpp` so kernels with identical
        # IR but different attrs don't collide. See also `_fast_caching/src_hasher.py` (`fn_attrs_hash`).
