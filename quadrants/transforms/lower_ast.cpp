 ...
 public:
  ...
  void visit(FrontendForStmt *stmt) override {
    ...
    if (stmt->snode) {
      ...
      // NOTE(amdgpu): MeshForStmt doesn't carry the force_inline hint, so its body never gets AlwaysInline on
      // AMDGPU. If a mesh-for ever needs to opt in, plumb force_inline through MeshForStmt and FrontendForStmt.
