...
void TaskCodeGenLLVM::visit(ExternalPtrStmt *stmt) {
  ...
  */
  ...
  for (int i = 0; i < num_array_args; i++) {
    ...
    // Fork: keep sizes in i64. Linear index below is i64, so doing the sign-extend here avoids a deferred SExt and
    // keeps the IR uniform on the hot path.
  ...
  for (int i = 0; i < num_indices; i++) {
    if (i >= element_shape_index_offset && i < element_shape_index_offset + num_element_indices) {
      ...
      // Fork: explicitly i64-typed constant so the following CreateMul against the i64 `linear_index` doesn't need
      // an SExt fixup.
      llvm::Value *size_var =
          tlctx->get_constant(get_data_type<int64>(), stmt->element_shape[i - element_shape_index_offset]);
  ...
  */
  if (operand_dtype->is<TensorType>()) {
    ...
    // Fork: linear_index is already i64 (sizes were SExt'd above, size_var was emitted as i64). No extra SExt needed.
...
std::string TaskCodeGenLLVM::init_offloaded_task_function(OffloadedStmt *stmt, std::string suffix) {
  ...
  // Fork: AMDGPU disallows byval on the calling convention, so the AMDGPU codegen overrides
  // `kernel_argument_struct_in_kernarg()` to pass the struct directly in kernarg and stores it to an alloca at function
  // entry to obtain a pointer. Other backends keep the original pointer parameter type.
  ...
  task_function_type =
      llvm::FunctionType::get(llvm::Type::getVoidTy(*llvm_context), {context_param_type}, false);
