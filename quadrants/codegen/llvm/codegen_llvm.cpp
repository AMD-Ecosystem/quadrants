...
void TaskCodeGenLLVM::visit(ExternalPtrStmt *stmt) {
  ...
  */
  ...
  for (int i = 0; i < num_array_args; i++) {
    ...
    // Widen runtime shape entries to i64 so the linear-index multiply chain below stays i64. ndarray
    // .nelement() and the GEP base offset are already i64 in this fork; staying in i32 here causes
    // CreateMul / CreateAdd type mismatches against the i64 linear_index initialised below.
  ...
  */
  if (operand_dtype->is<TensorType>()) {
    ...
    // linear_index is already i64 in this fork (see the ndarray index-widening above), so no SExt needed.
