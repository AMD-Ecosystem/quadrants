...
bool is_extension_supported(Arch arch, Extension ext) {
  static std::unordered_map<Arch, std::unordered_set<Extension>> arch2ext = {
      ...
      // NOTE(amdgpu): bls is intentionally omitted. The BLS test suite
      // (test_bls.py, test_bls_assume_in_range.py) uses
      // `qd.root.pointer(...).dense(...)` which requires Extension::sparse,
      // a feature that has NOT been ported to AMDGPU. Declaring bls without
      // sparse makes those tests RUN and fail with "Pointer SNode is not
      // supported on this backend" at field-builder time instead of
      // cleanly skipping at the `@test_utils.test(require=qd.extension.bls)`
      // decorator. Add bls here once sparse SNode codegen lands for AMDGPU.
