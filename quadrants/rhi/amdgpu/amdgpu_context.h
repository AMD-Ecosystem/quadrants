...
class AMDGPUContext {
 private:
  ...
  // Per-thread "active" HIP stream used by the kernel launcher; defaults to
  // the legacy/null stream (0). The shared stream_pool_ is used by
  // qd.stream_parallel() to dispatch independent kernels concurrently.
