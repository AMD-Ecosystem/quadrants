...
void AmdgpuDevice::dealloc_memory(DeviceAllocation handle) {
  ...
  if (info.use_memory_pool) {
    // NOTE(amdgpu): use the async free wrapper, which transparently falls back to synchronous hipFree on devices
    // without memory-pool support (see AMDGPUDriver::mem_free_async).
