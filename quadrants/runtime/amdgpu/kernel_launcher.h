...
class KernelLauncher : public LLVM::KernelLauncher {
  ...
  struct Context {
    ...
    // Fork-specific: resolved kernel-function handles cached per launch context (used by the offloaded-task
    // launcher to skip the per-launch host-side function-resolution path).
 ...
 private:
  // NOTE(npoulad/quadrants-0.8.0-rebase): adopted upstream signature so the new adstack lazy-claim + max-reducer
  // dispatch machinery in launch_offloaded_tasks is reachable. The fork's perf optimization (cached resolved_funcs +
  // hoisted arg vectors via the bundled Context &launcher_ctx) regressed in this file as a result. Resolved-funcs
  // cache survives in `Context::resolved_funcs` and should be re-wired into the new launch path as a follow-up.
